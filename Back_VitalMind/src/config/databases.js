...
// Agrega columnas a una tabla solo si todavía no existen, consultando
// information_schema. Funciona en cualquier versión de MySQL/MariaDB, a
// diferencia de "ADD COLUMN IF NOT EXISTS" (solo MySQL 8.0.29+).
async function ensureColumns(pool, table, columns) {
  ...
  const alterSql = `ALTER TABLE ${table} ${missing.map((c) => `ADD COLUMN ${c.name} ${c.ddl}`).join(", ")}`;
...
export async function connectDatabases() {
  if (!mysqlPool) {
    ...
    await adminConnection.query(`CREATE DATABASE IF NOT EXISTS \`${env.MYSQL_DATABASE}\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`);
    ...
    `);
    // Migración segura: agrega los campos de información personal usados en el
    // perfil (grupo sanguíneo, teléfono, peso, altura) si la tabla ya existía
    // de una instalación previa sin estas columnas. Se usa information_schema
    // en vez de "ADD COLUMN IF NOT EXISTS" porque esa sintaxis solo existe en
    // MySQL 8.0.29+ y no es compatible con MariaDB ni versiones anteriores.
    ...
    await mysqlPool.query(`
      CREATE TABLE IF NOT EXISTS notifications (
        id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
        user_id BIGINT UNSIGNED NULL,
        kind ENUM('tip','reminder','ai','alert') NOT NULL DEFAULT 'tip',
    ...
    await mysqlPool.query(`
      ...
      PRIMARY KEY (id),
        ...
        UNIQUE KEY uq_emotional_logs_user_day (user_id, log_date),
...
export function getDatabaseStatus() {
  return { mysql: { connected: Boolean(mysqlPool) } };
}
